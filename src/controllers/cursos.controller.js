...
function agregarCurso(req, res) {
    ...
    if (parametros.nombreCurso) {
        ...
        if (!underscore.isEmpty(parametros.descripcion)) cursoModel.descripcion = parametros.descripcion
...
function buscarCursosCreados(req, res) {
    Curso.find({ idProfesor: req.user.sub }, (err, cursosEncontrados) => {
        if (err) return res.status(404).send({ mensaje: 'Error en la peticion' });
        if (!cursosEncontrados) return res.status(404).send({ mensaje: 'No posee ningun curso' })
